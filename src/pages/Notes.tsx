...
export default function Notes() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const snap = await getDocs(query(collection(db, "users", profile.id, "notes")));
  ...
  const openEditor = (note?: Note) => {
    if (note) { setEditNote(note); setEditTitle(note.title); setEditContent(note.content); setEditShared(note.isShared || false); }
    else { setEditNote(null); setEditTitle(""); setEditContent(""); setEditShared(false); }
    setEditing(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editNote) {
        await updateDoc(doc(db, "users", profile.id, "notes", editNote.id), { title: editTitle.trim(), content: editContent.trim(), isShared: editShared, updatedAt: new Date().toISOString() });
        setNotes((prev) => prev.map((n) => n.id === editNote.id ? { ...n, title: editTitle.trim(), content: editContent.trim(), isShared: editShared, updatedAt: new Date().toISOString() } : n));
      } else {
        const docRef = await addDoc(collection(db, "users", profile.id, "notes"), { title: editTitle.trim(), content: editContent.trim(), isShared: editShared, authorRole: "client", createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() });
        setNotes((prev) => [{ id: docRef.id, title: editTitle.trim(), content: editContent.trim(), isShared: editShared, authorRole: "client", createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }, ...prev]);
      }
      setEditing(false);
    ...
    <PageTransition className="min-h-screen bg-background">
      ...
      <StaggerContainer className="max-w-2xl mx-auto px-5 py-6 space-y-5">
        <StaggerItem>
          <div className="relative">
            ...
            <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Notizen durchsuchen..." className="w-full pl-11 pr-4 py-3.5 bg-card rounded-2xl border border-border text-foreground font-medium focus:outline-none focus:ring-2 focus:ring-ring" />
          </div>
        </StaggerItem>
        ...
        {filtered.length === 0 ? (
          <StaggerItem>
            <div className="text-center py-16">
              ...
              <h2 className="text-xl font-black text-foreground mb-2">{searchTerm ? "Keine Treffer" : "Noch keine Notizen"}</h2>
              <p className="text-muted-foreground">{searchTerm ? "Versuche einen anderen Suchbegriff." : "Erstelle deine erste Notiz mit dem + Button."}</p>
            </div>
          </StaggerItem>
        ) : (
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1 pr-3">
                            ...
                            <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                              ...
                              {isMine && note.isShared && <span className="text-[10px] font-extrabold bg-primary/10 text-primary px-2 py-0.5 rounded-md">Freigegeben</span>}
                            </div>
                          </div>
      ...
      <AnimatePresence>
        {editing && (
          ...
          >
            <motion.div
              className="bg-card rounded-3xl border border-border w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl"
            ...
            >
              <div className="flex items-center justify-between p-6 border-b border-border">
                <h2 className="text-lg font-black text-foreground">{editNote ? "Notiz bearbeiten" : "Neue Notiz"}</h2>
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="flex items-center justify-between bg-secondary rounded-2xl border border-border p-4">
                  ...
                  <motion.button
                    ...
                    className={`w-12 h-12 rounded-full flex items-center justify-center transition-all ${editShared ? "bg-primary/15 border-2 border-primary" : "bg-muted border border-border"}`}
                    whileTap={{ scale: 0.9 }}
                  >
      ...
      <AnimatePresence>
        {deleteTarget && (
          ...
          >
            <motion.div
              className="bg-card rounded-3xl border border-border w-full max-w-sm p-6 shadow-2xl text-center"
              initial={{ opacity: 0, scale: 0.9, y: 30 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.9, y: 30 }}
              transition={{ type: "spring", damping: 20, stiffness: 150 }}
            >
              ...
              <p className="text-sm text-muted-foreground mb-6">„{deleteTarget.title}" wird unwiderruflich gelöscht.</p>
