...
function getFileInfo(file: any) {
  ...
  if ([".jpg", ".jpeg", ".png", ".gif", ".webp"].some((e) => name.endsWith(e))) return { icon: ImageIcon, color: "#8B5CF6", bg: "bg-[#8B5CF6]/10", label: "Bild" };
  if ([".mp4", ".mov"].some((e) => name.endsWith(e))) return { icon: Film, color: "#F59E0B", bg: "bg-[#F59E0B]/10", label: "Video" };
  return { icon: FileText, color: "#64748B", bg: "bg-muted", label: "Datei" };
}
          ...
          <motion.div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <motion.div className="bg-card rounded-3xl border border-border w-full max-w-lg shadow-2xl" initial={{ opacity: 0, scale: 0.9, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 30 }} transition={{ type: "spring", damping: 20, stiffness: 150 }}>
              ...
              <div className="p-6 pt-0">
                <motion.button onClick={handleUpload} disabled={uploading || !title.trim()} className="w-full py-4 rounded-2xl bg-accent text-accent-foreground font-black text-lg disabled:opacity-40 flex items-center justify-center gap-2 shadow-lg" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}>
