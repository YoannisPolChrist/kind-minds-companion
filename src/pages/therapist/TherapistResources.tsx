...
function AddResourceModal({
  ...
  const handleUploadFile = async () => {
    ...
    setUploading(true);
    try {
      const filename = `resources/${Date.now()}_${selectedFile.name}`;
      ...
      reset();
      onClose();
      onSaved("Datei hochgeladen");
    } catch (err) {
      console.error(err);
      setError(err instanceof Error ? `Upload fehlgeschlagen: ${err.message}` : "Upload fehlgeschlagen.");
      onSaved("Upload fehlgeschlagen", "error");
    } finally {
      setUploading(false);
    }
  };
          ...
          <div className="space-y-4">
            ...
            {resourceType === "file" && (
              <div className="space-y-3">
                <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider block">Datei *</label>
                <input
                  ref={fileInputRef}
                  type="file"
                  className="hidden"
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        {selectedFile
                          ? `${Math.max(1, Math.round(selectedFile.size / 1024))} KB bereit für den Upload`
                          : "Datei bleibt auch bei Fehlern im Formular erhalten."}
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={() => fileInputRef.current?.click()}
                      className="inline-flex items-center justify-center gap-2 rounded-2xl bg-card px-4 py-3 text-sm font-bold text-foreground border border-border hover:bg-muted transition-colors"
                    >
                      <Upload size={16} />
                      {selectedFile ? "Datei wechseln" : "Datei wählen"}
                    </button>
                  </div>
                </div>
              </div>
  ...
  const confirm = async () => {
    ...
    try {
      ...
      for (const clientId of selected) {
        ...
        promises.push(
          addDoc(collection(db, "notifications"), buildNotificationDocumentData({
            userId: clientId,
            type: "resource_shared",
            resourceTitle: resourcesToAssign.length === 1 ? resourcesToAssign[0]?.title : undefined,
            resourceType: resourcesToAssign.length === 1 ? resourcesToAssign[0]?.type : undefined,
            resourceCount: resourcesToAssign.length,
            createdAt: serverTimestamp(),
          }))
        );
      }
...
function PreviewModal({ resource, onClose }: { resource: any; onClose: () => void }) {
  ...
  return (
    <motion.div className="fixed inset-0 z-50 flex items-end md:items-center md:justify-center md:p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={onClose} />
      <motion.div
        className="relative mt-auto flex w-full flex-col overflow-hidden bg-background rounded-t-[2rem] max-h-[92vh] md:mt-0 md:max-w-6xl md:rounded-[2rem] md:border md:border-border md:shadow-2xl"
        initial={{ y: "100%" }}
        animate={{ y: 0 }}
      ...
      >
        ...
        <div className="m-4 mb-8 flex-1 overflow-hidden rounded-[2rem] border border-border bg-secondary/50 min-h-[300px] md:m-6 md:min-h-[540px]">
          {resource.type === "image" ? (
...
export default function TherapistResources() {
  ...
  const pinnedCount = useMemo(() => resources.filter((res) => res.isPinned).length, [resources]);
  ...
  return (
    <PageTransition className="min-h-screen bg-background">
      ...
      <TherapistHeroHeader maxWidthClassName="max-w-6xl" imageIndex={2}>
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-[11px] font-black uppercase tracking-[0.24em] text-white/70">Therapeut:innenbereich</p>
            <h1 className="mt-3 text-2xl font-black tracking-tight text-white">Bibliothek & Materialien</h1>
            <div className="mt-4 flex flex-wrap gap-2">
              <span className="rounded-2xl border border-white/20 px-3 py-1.5 text-[11px] font-black uppercase tracking-[0.22em] text-white/75">
                {resourceTotal} Ressourcen
              </span>
              <span className="rounded-2xl border border-white/20 px-3 py-1.5 text-[11px] font-black uppercase tracking-[0.22em] text-white/75">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <motion.button
              ...
              className="flex items-center gap-2 bg-white/15 hover:bg-white/25 px-4 py-2.5 rounded-2xl text-sm font-bold text-white"
              whileHover={{ x: -3 }}
              whileTap={{ scale: 0.95 }}
            >
            ...
            <PressableScale onClick={() => setAddModalOpen(true)}>
              <div className="flex items-center gap-2 bg-white/15 hover:bg-white/25 px-4 py-2.5 rounded-2xl text-sm font-bold text-white transition-colors">
        ...
        <div className="mt-6 relative">
          ...
          <input
            ...
            className="w-full pl-12 pr-10 py-4 bg-white/12 border border-white/25 rounded-2xl text-white font-bold placeholder-white/50 focus:outline-none text-lg"
          />
          {searchQuery && (
