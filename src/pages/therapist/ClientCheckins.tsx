...
export default function ClientCheckins() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const snap = await getDocs(query(collection(dbLite, "checkins"), where("uid", "==", id)));
  ...
  const grouped = useMemo(() => {
    ...
    checkins.forEach((ci: any) => {
      ...
      const key = new Date(dateStr).toLocaleDateString(locale, { day: "2-digit", month: "long", year: "numeric" });
              ...
              <div className="flex items-end gap-1 h-28">
                {checkins.slice(0, 30).reverse().map((ci: any, i: number) => (
                  <motion.div
                    ...
                    title={`${ci.mood}/10 - ${new Date(ci.createdAt || ci.date).toLocaleDateString(locale)}`}
                  />
                ))}
              </div>
            </div>
                  ...
                  <div key={ci.id} className="bg-card rounded-2xl border border-border p-4 shadow-sm">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {ci.createdAt && new Date(ci.createdAt).toLocaleTimeString(locale, { hour: "2-digit", minute: "2-digit" })}
                      </span>
                    </div>
                    {ci.tags?.length > 0 && (
