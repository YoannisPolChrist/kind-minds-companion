...
const BuilderChartPreview = lazy(() => import("../../components/therapist/BuilderChartPreview"));
const ExerciseFlowTimeline = lazy(() => import("../../components/charts/ExerciseFlowTimeline"));
...
function ChartOptionEditor({ block, onChange, chartType }: {
  ...
  return (
    <>
      ...
      <Suspense
        fallback={
          <div className="mt-4 rounded-2xl border border-border bg-secondary/50 p-4 text-center text-sm font-semibold text-muted-foreground">
            Vorschau wird geladen...
          </div>
        }
      >
        <BuilderChartPreview chartType={chartType} options={block.options || []} />
      </Suspense>
    </>
  );
}
  ...
  return (
    <PageTransition className="min-h-screen bg-background pb-32">
      ...
      <div className="max-w-3xl mx-auto px-6 py-6">
        ...
        {blocks.length > 0 && (
          <div className="mb-6">
            <Suspense fallback={<div className="rounded-2xl border border-border bg-secondary/50 p-4 text-sm font-semibold text-muted-foreground">Ablauf wird geladen...</div>}><ExerciseFlowTimeline blocks={blocks} /></Suspense>
          </div>
        )}
...
function CatalogueModal({ onAdd, onClose }: { onAdd: (type: BlockType) => void; onClose: () => void }) {
  ...
  return (
    <motion.div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center p-0 sm:p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}>
      <motion.div
