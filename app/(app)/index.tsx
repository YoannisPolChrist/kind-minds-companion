import { View, Text, TouchableOpacity, ActivityIndicator, InteractionManager, Linking, Platform, ScrollView, useWindowDimensions } from 'react-native';
...
import { TranslatorAssist } from '../../components/ui/TranslatorAssist';
...
export default function ClientDashboard() {
    ...
    useFocusEffect(
        useCallback(() => {
            ...
            load();
            return () => { active = false; };
        }, [profile?.id, fetchExercises, fetchCheckinStatus, fetchNotifications])
    );

    const fetchNotifications = useCallback(async () => {
        if (!profile?.id) return;
        try {
            setNotifications(await fetchUnreadNotifications(profile.id));
        } catch (e) {
    ...
    const handleNotificationClick = useCallback(async () => {
        ...
        try {
            await markNotificationsAsRead(notifications.map(n => n.id), profile?.id);
            setNotifications([]);
            router.push((primaryNotification?.appPath || '/(app)') as any);
        } catch (e) {
            console.error('Error updating notifications', e);
        }
    }, [notifications, primaryNotification?.appPath, profile?.id, router]);
    ...
    const quickActionWidth = screenWidth < 700 ? '100%' : '33.33%';
    const quickActions = useMemo(() => ([
        {
            key: 'overview',
            eyebrow: 'Insights',
            title: 'Daten & Verlauf',
            description: 'Fortschritt, Uebungen und Stimmungsverlauf im Ueberblick.',
            accent: '#0EA5E9',
            surface: isDark ? 'rgba(15,23,42,0.65)' : '#F5FBFF',
            onPress: () => router.push('/(app)/exercises_overview' as any),
        },
        {
            key: 'checkin',
            eyebrow: checkedInToday ? 'Heute erledigt' : 'Heute offen',
            title: checkedInToday ? 'Check-in ansehen' : 'Check-in starten',
            description: checkedInToday ? 'Deinen heutigen Eintrag und Verlauf oeffnen.' : 'Kurzer Tagesimpuls fuer Stimmung und Fokus.',
            accent: '#DB2777',
        ...
        },
        {
            ...
            eyebrow: 'Bibliothek',
            title: 'Ressourcen',
            description: 'Dateien, Links und Materialien von deinem Coach.',
            accent: isDark ? '#A78BFA' : colors.secondary,
            surface: isDark ? 'rgba(255,255,255,0.05)' : '#FFFFFF',
            onPress: () => router.push('/(app)/resources' as any),
        },
        {
            key: 'journal',
            eyebrow: 'Session-Tagebuch',
            title: 'Journal',
            description: 'Erkenntnisse, Notizen und Session-Reflexion festhalten.',
            accent: '#3B82F6',
            surface: isDark ? 'rgba(255,255,255,0.05)' : '#FFFFFF',
            onPress: () => router.push('/(app)/notes' as any),
        },
    ]), [checkedInToday, router, isDark, colors.secondary]);

    const shouldUseCarousel = screenWidth < 960;
    const quickActionCardWidth = shouldUseCarousel ? Math.min(screenWidth * 0.78, 320) : undefined;
    const quickActionCardHeight = shouldUseCarousel ? 190 : 172;
    const currentHour = new Date().getHours();
    const isMorningWindow = currentHour >= 5 && currentHour < 12;
    const isEveningWindow = currentHour >= 18 && currentHour < 24;
    const showCheckinPrompt = !checkedInToday && (isMorningWindow || isEveningWindow);
            ...
            >
                ...
                >
                    <Image
                        ...
                        contentFit="cover"
                    />
                    <TriangulumBackdrop variant={isDark ? 'dark' : 'light'} />
                    <LightAmbientOrbs />
                    ...
                    <View style={{ zIndex: 10, maxWidth: 680, width: '100%', alignSelf: 'center' }} pointerEvents="box-none">
                        ...
                        >
                            ...
                            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: exercises.length > 0 ? 16 : 0 }}>
                                <View style={{ paddingHorizontal: 12, paddingVertical: 8, borderRadius: 999, backgroundColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.68)', borderWidth: 1, borderColor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.04)' }}>
                                    <Text style={{ color: colors.text, fontSize: 12, fontWeight: '800' }}>
                                        {checkedInToday ? 'Check-in erledigt' : 'Check-in offen'}
                                    </Text>
                                </View>
                                <View style={{ paddingHorizontal: 12, paddingVertical: 8, borderRadius: 999, backgroundColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.68)', borderWidth: 1, borderColor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.04)' }}>
                                    <Text style={{ color: colors.text, fontSize: 12, fontWeight: '800' }}>
                                        {openExercises.length} offene Aufgaben
                                    </Text>
                                </View>
                                {notifications.length > 0 && (
                                    <View style={{ paddingHorizontal: 12, paddingVertical: 8, borderRadius: 999, backgroundColor: isDark ? 'rgba(56,189,248,0.18)' : 'rgba(14,165,233,0.12)', borderWidth: 1, borderColor: isDark ? 'rgba(56,189,248,0.22)' : 'rgba(14,165,233,0.18)' }}>
                                        <Text style={{ color: isDark ? '#7DD3FC' : '#0369A1', fontSize: 12, fontWeight: '800' }}>
                                            {notifications.length} neue Nachricht{notifications.length > 1 ? 'en' : ''}
                                        </Text>
                                    </View>
                                )}
                            </View>
                            ...
                            {showCheckinPrompt && (
                                <MotiView
                                    ...
                                    transition={{ type: 'spring', damping: 16, stiffness: 150, delay: 60 }}
                                    style={{ alignItems: 'center', marginBottom: exercises.length > 0 ? 16 : 10 }}
                                >
                                    <PressableScale
                                        scaleTo={0.94}
                                        onPress={() => router.push('/(app)/checkin' as any)}
                                        style={{ width: screenWidth < 360 ? 190 : 230, height: screenWidth < 360 ? 190 : 230 }}
                                    >
                                        <View
                                            style={{
                                                flex: 1,
                                        ...
                                        >
                                            <LinearGradient
                                                colors={isDark ? ['#0f1f24', '#122d33'] : ['#FFFFFF', '#F5FBFF']}
                                                style={{ position: 'absolute', inset: 0 }}
                                            />
                                            <MotiView
                                                from={{ opacity: 0.4, scale: 0.85 }}
                                                animate={{ opacity: 0.85, scale: 1.05 }}
                                                transition={{ loop: true, repeatReverse: true, delay: 2000, duration: 2600 }}
                                                style={{
                                                    position: 'absolute',
                                                    inset: 12,
                                                    borderRadius: 9999,
                                                    borderWidth: 2,
                                                    borderColor: isDark ? 'rgba(192,157,89,0.6)' : 'rgba(19,115,134,0.5)',
                                                }}
                                            />
                                            <Text style={{ color: colors.textSubtle, fontSize: 12, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 1.4 }}>
                                                Check-in
                                            </Text>
                                            <Text style={{ color: colors.text, fontSize: 28, fontWeight: '900', marginTop: 6, letterSpacing: -0.6 }}>
                                                Jetzt starten
                                            </Text>
                                            <Text style={{ color: colors.textSubtle, fontSize: 12, fontWeight: '600', marginTop: 8 }}>
                                                {isMorningWindow ? 'Morgens' : 'Abends'}
                                            </Text>
                                            <MotiView
                                                from={{ rotate: '0deg' }}
                                                animate={{ rotate: '360deg' }}
                                                transition={{ loop: true, duration: 10000, easing: () => 1 }}
                                                style={{
                                                    position: 'absolute',
                                                    width: '60%',
                                                    height: 2,
                                                    backgroundColor: isDark ? 'rgba(19,115,134,0.6)' : 'rgba(19,115,134,0.4)',
                                                }}
                                            />
                                        </View>
                                    </PressableScale>
                ...
                <View style={[contentMaxWidth ? { maxWidth: contentMaxWidth, width: '100%', alignSelf: 'center' } : undefined, { paddingHorizontal: horizPadding }]}>
                            ...
                            >
                                ...
                                <View style={{ flex: 1 }}>
                                    ...
                                    <Text style={{ color: isDark ? '#7DD3FC' : '#0284C7', fontSize: 13 }} numberOfLines={2}>
                                        {primaryNotification?.body || primaryNotification?.message || 'Dein Therapeut hat neue Ressourcen für dich freigeschaltet.'}
                                    </Text>
                                </View>
                            ...
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}>
                                <View style={{ flex: 1, minWidth: 220 }}>
                                    <Text style={{ color: colors.text, fontSize: 22, fontWeight: '900', letterSpacing: -0.5, marginBottom: 4 }}>
                                        {i18n.t('dashboard.quick_access_title', { defaultValue: 'Meine Lieben' })}
                                    </Text>
                                    <Text style={{ color: colors.textSubtle, fontSize: 14, lineHeight: 20 }}>
                                        {i18n.t('dashboard.quick_access_desc', { defaultValue: 'Deine drei wichtigsten Bereiche auf einen Blick.' })}
                                    </Text>
                                </View>
                                <TranslatorAssist variant="pill" />
                            </View>
                        </View>

                        {shouldUseCarousel ? (
                            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 6, paddingLeft: 6, paddingRight: 6 }}>
                                {quickActions.map((action, index) => (
                                    <MotiView
                                        ...
                                        style={{
                                            ...
                                            marginRight: index === quickActions.length - 1 ? 0 : 12,
                                        }}
                                    >
                                        <PressableScale onPress={action.onPress}>
                                            <View
                                                style={{
                                                    minHeight: quickActionCardHeight,
                                            ...
                                            >
                                                <View style={{ width: 46, height: 4, borderRadius: 999, backgroundColor: action.accent, marginBottom: 18 }} />
                                                <Text style={{ color: action.accent, fontSize: 11, fontWeight: '800', letterSpacing: 1.1, textTransform: 'uppercase', marginBottom: 10 }}>
                                                    {action.eyebrow}
                                                </Text>
                                                ...
                                                <View style={{ marginTop: 18, alignSelf: 'flex-start', paddingHorizontal: 14, paddingVertical: 8, borderRadius: 999, backgroundColor: isDark ? 'rgba(255,255,255,0.05)' : '#F7F4EE' }}>
                                                    <Text style={{ color: action.accent, fontSize: 12, fontWeight: '800' }}>
                                                        Oeffnen
                                                    </Text>
                                                </View>
                                            </View>
                            ...
                            <View style={{ flexDirection: 'row', gap: 12 }}>
                                {quickActions.map((action, index) => (
                                    <MotiView
                                        ...
                                        animate={{ opacity: 1, translateY: 0, scale: 1 }}
                                        transition={{ type: 'timing', duration: 320, delay: 120 + (index * 60) }}
                                        style={{ flex: 1 }}
                                    >
                                        <PressableScale onPress={action.onPress}>
                                            <View
                                            ...
                                            >
                                                ...
                                                <Text style={{ color: colors.text, fontSize: 17, fontWeight: '900', marginBottom: 6 }}>
                                                    {action.title}
                                                </Text>
                                                <Text style={{ color: colors.textSubtle, fontSize: 13, lineHeight: 19, flex: 1 }}>
                                                    {action.description}
                                                </Text>
                                                <View style={{ marginTop: 16, alignSelf: 'flex-start', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 999, backgroundColor: isDark ? 'rgba(255,255,255,0.05)' : '#F7F4EE' }}>
                                                    <Text style={{ color: action.accent, fontSize: 12, fontWeight: '800' }}>
                                                        Oeffnen
                                                    </Text>
                                                </View>
                    ...
                    {exercises.length > 0 && (
                        ...
                        >
                            <View style={{ marginBottom: 12 }}>
                                <Text style={{ color: colors.text, fontSize: 20, fontWeight: '900', letterSpacing: -0.4, marginBottom: 4 }}>
                                    {i18n.t('dashboard.overview_title', { defaultValue: 'Mein Ueberblick' })}
                                </Text>
                    ...
                    <MotiView
                        ...
                        style={{ marginBottom: 24, display: 'none' }}
                    >
                        <PressableScale
                            onPress={() => {
