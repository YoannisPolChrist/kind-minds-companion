...
import { deriveExerciseStatus, isExerciseActive, isExerciseDone } from "../../../../../utils/domain/exercises";
import { TherapistClientScreenHeader } from "../../../../../components/therapist/client/TherapistClientScreenHeader";

const REMINDER_PRESETS = ["08:00", "12:30", "18:00", "20:15"];
...
function ReminderTimeSection({
  ...
  const hasValidTime = isValidReminderTime(reminderTime);
    ...
    <View style={{ marginTop: 8 }}>
      ...
      >
        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
          <View>
            <Text style={{ color: "#243842", fontWeight: "800", fontSize: 14 }}>
              Erinnerung senden
            </Text>
            <Text style={{ color: "#64748B", fontSize: 12, marginTop: 4 }}>
              24h-Format, z. B. 18:00
            </Text>
          </View>
          <View
            style={{
              ...
              paddingVertical: 8,
              borderRadius: 999,
              backgroundColor: hasValidTime ? "rgba(19,115,134,0.10)" : "#F8FAFC",
              borderWidth: 1,
              borderColor: hasValidTime ? "rgba(19,115,134,0.18)" : "#E2E8F0",
            }}
          >
            <Text style={{ color: hasValidTime ? "#137386" : "#94A3B8", fontWeight: "800", fontSize: 12 }}>
              {hasValidTime ? `${reminderTime} Uhr` : "Zeit waehlen"}
            </Text>
          </View>
        </View>
        ...
        >
          ...
          <TextInput
            value={reminderTime}
            onChangeText={(value) => onReminderTimeChange(formatReminderTimeInput(value))}
            placeholder="18:00"
            keyboardType="numeric"
            maxLength={5}
            style={{ flex: 1, fontWeight: "900", fontSize: 24, color: "#243842", letterSpacing: 1.5 }}
          />
          <Text style={{ color: "#94A3B8", fontWeight: "700", fontSize: 14, textTransform: "uppercase", letterSpacing: 1.5, marginLeft: 12 }}>
            Uhr
          </Text>
        </View>
        ...
        <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 10, marginTop: 14 }}>
          {REMINDER_PRESETS.map((preset) => {
            const isActive = reminderTime === preset;
            return (
              <TouchableOpacity
                key={preset}
                onPress={() => onReminderTimeChange(preset)}
                style={{
                  paddingHorizontal: 14,
                  paddingVertical: 10,
                  borderRadius: 16,
                  borderWidth: 1,
                  borderColor: isActive ? "#137386" : "#E2E8F0",
                  backgroundColor: isActive ? "rgba(19,115,134,0.08)" : "#F8FAFC",
                }}
              >
                <Text style={{ color: isActive ? "#137386" : "#475569", fontWeight: "800", fontSize: 13 }}>
                  {preset}
                </Text>
              </TouchableOpacity>
            );
...
export default function ClientExercisesView() {
  ...
  const saveExercise = async (title: string, blocks: ExerciseBlock[], themeColor?: string, coverImage?: string) => {
    try {
      ...
      await ExerciseRepository.create(sanitizedData);
  ...
  return (
    <View className="flex-1 bg-[#FAF9F6]">
      <TherapistClientScreenHeader
        ...
        title="Uebungen"
        subtitle="Aufgaben zuweisen, Status verfolgen und Fortschritt sichtbar machen."
        onBack={goBack}
        seedSuffix="exercises"
        actions={[
          {
            key: "assign",
            label: "Uebung zuweisen",
            icon: <Activity size={16} color="#137386" />,
        ...
        stats={[
          {
            key: "open",
            label: "Offen",
            value: exercises.filter((e) => isExerciseActive(e)).length,
          },
          {
            key: "done",
            label: "Erledigt",
            value: exercises.filter((e) => isExerciseDone(e)).length,
          },
        ]}
      />
        ...
        <View className="w-full max-w-4xl mx-auto">
              ...
              <View className="bg-orange-50 px-4 py-2 rounded-[16px] border border-orange-100/50 shadow-sm">
                <Text className="text-orange-600 text-[13px] font-black tracking-wide uppercase">
                  {i18n.t("therapist.open_exercises", {
                    count: exercises.filter((e) => isExerciseActive(e)).length,
                  })}
                </Text>
              </View>
              <View className="bg-emerald-50 px-4 py-2 rounded-[16px] border border-emerald-100/50 shadow-sm">
                <Text className="text-emerald-600 text-[13px] font-black tracking-wide uppercase">
                  {i18n.t("therapist.done_exercises", {
                    count: exercises.filter((e) => isExerciseDone(e)).length,
                  })}
                </Text>
              </View>
            </View>
          ...
          {exercises.length === 0 ? (
            ...
            <View className="mb-6">
              ...
              {exercises.filter((e) => isExerciseActive(e)).length > 0 && (
                <View className="mb-8">
                              ...
                              <View className="flex-row flex-wrap gap-2 mt-1">
                                <View className="px-3 py-1.5 rounded-xl self-start bg-orange-50 border border-orange-100/50">
                                  <Text className="text-[13px] font-bold text-orange-600">
                                    {deriveExerciseStatus(ex) === "assigned" ? "Zugewiesen" : i18n.t("therapist.status_open")}
                                  </Text>
                                </View>
              ...
              {exercises.filter((e) => isExerciseDone(e)).length > 0 && (
                <View className="mb-4">
                  <Text className="text-[13px] font-black text-[#243842]/40 uppercase tracking-widest mb-4 ml-2">
