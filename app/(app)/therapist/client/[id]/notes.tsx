...
import { Edit3, Plus, X, Calendar, ChevronRight, Search, Trash2, Camera, Lock, BookOpen } from 'lucide-react-native';
...
const NoteCard = React.memo(({ note, isExpanded, onToggleExpand, onDeletePrompt, onToggleShare, formatTime }: any) => {
            ...
            >
                ...
                {isExpanded && (
                    ...
                    >
                        ...
                        {!session && note.authorRole === 'therapist' && (
                            <View style={{ flexDirection: 'row', gap: 12, marginTop: 24, paddingTop: 16, borderTopWidth: 1, borderTopColor: '#F8FAFC' }}>
                                <TouchableOpacity
                                    onPress={onToggleShare}
                                    style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: note.isShared ? '#EEF2FF' : '#F8FAFC', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 12 }}
                                >
                                    <BookOpen size={14} color={note.isShared ? '#4F46E5' : '#64748B'} />
                                    <Text style={{ fontSize: 13, fontWeight: '700', color: note.isShared ? '#4F46E5' : '#64748B', marginLeft: 6 }}>
                                        {note.isShared ? 'Freigegeben' : 'Privat'}
                                    </Text>
                                </TouchableOpacity>
                                <TouchableOpacity
                                    onPress={onDeletePrompt}
...
export default function TherapistClientNotesScreen() {
    ...
    const [newEntryType, setNewEntryType] = useState<'session' | 'journal'>('session');
    ...
    const fetchNotes = async () => {
        if (!id) return;
        try {
            const data = await NoteRepository.findForTherapist(id as string);
            setNotes(data);
        } catch (error) {
    ...
    const handleSaveNote = async () => {
        ...
        try {
            ...
            await NoteRepository.create({
                ...
                isShared: newEntryType === 'journal' ? newEntryShared : false,
            });

            setNewNoteContent('');
            setNewNoteTitle('');
            setNewNoteImage(null);
            setNewEntryType('session');
            ...
            setToast({
                visible: true,
                message: 'Gespeichert',
                subMessage: newEntryType === 'session' ? 'Session Note wurde gespeichert.' : 'Journal-Eintrag wurde gespeichert.',
                type: 'success'
            });
        } catch (error) {
    ...
    const handleToggleShare = async (note: any) => {
        try {
            await NoteRepository.setShared(note.id, !note.isShared);
            setNotes(prev => prev.map(entry => entry.id === note.id ? { ...entry, isShared: !note.isShared } : entry));
            setToast({
                visible: true,
                message: !note.isShared ? 'Freigegeben' : 'Privat',
                subMessage: !note.isShared ? 'Der Eintrag ist jetzt fuer den Klienten sichtbar.' : 'Der Eintrag ist wieder nur fuer dich sichtbar.',
                type: 'success'
            });
        } catch (error) {
            const { message } = ErrorHandler.handle(error, 'Toggle Journal Sharing');
            setToast({ visible: true, message: 'Fehler', subMessage: message, type: 'error' });
        }
    };
    ...
    return (
        <View style={{ flex: 1, backgroundColor: '#F9F8F6' }}>
            <TherapistClientScreenHeader
                clientId={id as string}
                title="Journal"
                subtitle="Session Notes, private Gedanken und freigegebene Eintraege."
                onBack={goBack}
                seedSuffix="journal"
                actions={[
                    {
                        key: 'new-entry',
                        label: 'Neuer Eintrag',
                        icon: <Plus size={16} color="#137386" />,
                        onPress: () => {
                            setNewEntryType('session');
                            setNewEntryShared(false);
                ...
                stats={[
                    { key: 'session', label: 'Session', value: sessionCount },
                    { key: 'journal', label: 'Journal', value: journalCount },
                ]}
            />
                ...
                <View style={{ flexDirection: 'row', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
                    {([
                        ...
                        <TouchableOpacity
                            ...
                            style={{
                                backgroundColor: filter === key ? '#137386' : '#ffffff',
                                paddingHorizontal: 14,
                                paddingVertical: 8,
                                borderRadius: 16,
                                borderWidth: 1,
                                borderColor: filter === key ? '#137386' : '#E5E7EB',
                            }}
                        >
                            <Text style={{ color: filter === key ? 'white' : '#0F172A', fontWeight: '700', fontSize: 13 }}>{label}</Text>
                        </TouchableOpacity>
                    ))}
                </View>
                ...
                <View style={{ backgroundColor: '#137386', paddingTop: 64, paddingBottom: 28, paddingHorizontal: 28 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                        ...
                        >
                            ...
                            <Text style={{ color: '#137386', fontWeight: '800', marginLeft: 6, fontSize: 15 }}>Neuer Eintrag</Text>
                        </TouchableOpacity>
                    </View>

                    <Text style={{ color: 'white', fontSize: 28, fontWeight: '900', letterSpacing: -0.5, marginBottom: 4 }}>Journal & Session Notes</Text>
                ...
                <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', padding: 40 }}>
                    <MotiView from={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 100, type: 'spring' }} style={{ alignItems: 'center' }}>
                        ...
                        <Text style={{ fontSize: 15, color: '#64748B', textAlign: 'center', lineHeight: 22, maxWidth: 300, fontWeight: '500', marginBottom: 32 }}>
                            Halte Beobachtungen, Session Notes und freigebbare Journal-Eintraege fest.
                        </Text>
                        <TouchableOpacity
                            onPress={() => {
                                setNewEntryType('session');
                        ...
                        >
                            ...
                            <Text style={{ color: 'white', fontWeight: '800', fontSize: 15, marginLeft: 8 }}>Ersten Eintrag erstellen</Text>
                        </TouchableOpacity>
                    </MotiView>
                </View>
                ...
                >
                    <View style={{ flex: 1, paddingTop: Platform.OS === 'android' ? 60 : 20, backgroundColor: '#ffffff' }}>
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 24, paddingBottom: 16 }}>
                            ...
                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                                <TouchableOpacity
                                    ...
                                    style={{ flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: newEntryType === 'session' ? '#EEF7F8' : '#F8FAFC', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12 }}
                                >
                                    <Lock size={12} color={newEntryType === 'session' ? '#137386' : '#64748B'} />
                                    <Text style={{ fontSize: 12, fontWeight: '800', color: newEntryType === 'session' ? '#137386' : '#64748B' }}>Session</Text>
                                </TouchableOpacity>
                                <TouchableOpacity
                                    onPress={() => setNewEntryType('journal')}
                                    style={{ flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: newEntryType === 'journal' ? '#EEF2FF' : '#F8FAFC', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12 }}
                                >
                                    <BookOpen size={12} color={newEntryType === 'journal' ? '#4F46E5' : '#64748B'} />
                                    <Text style={{ fontSize: 12, fontWeight: '800', color: newEntryType === 'journal' ? '#4F46E5' : '#64748B' }}>Journal</Text>
                                </TouchableOpacity>
                            </View>
                        ...
                        <TextInput
                            ...
                            placeholder={newEntryType === 'session' ? 'Session Title...' : 'Journal Title...'}
                            placeholderTextColor="#CBD5E1"
                            value={newNoteTitle}
                            onChangeText={setNewNoteTitle}
                        />

                        {newEntryType === 'journal' && (
                            <View style={{ paddingHorizontal: 24, paddingBottom: 12 }}>
                                <TouchableOpacity
                                    onPress={() => setNewEntryShared(!newEntryShared)}
                                    style={{ alignSelf: 'flex-start', flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: newEntryShared ? '#EEF2FF' : '#F8FAFC', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 12 }}
                                >
                                    {newEntryShared ? <BookOpen size={12} color="#4F46E5" /> : <Lock size={12} color="#64748B" />}
                                    <Text style={{ fontSize: 12, fontWeight: '800', color: newEntryShared ? '#4F46E5' : '#64748B' }}>
                                        {newEntryShared ? 'Fuer Klienten freigegeben' : 'Nur fuer Therapeut sichtbar'}
                                    </Text>
                                </TouchableOpacity>
                            </View>
                        )}
