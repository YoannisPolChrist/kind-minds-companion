...
import { doc, setDoc, serverTimestamp, updateDoc, getDocs, query, collection, where, addDoc, writeBatch, getDoc } from 'firebase/firestore';
...
export function useAuthActions(): AuthResponse {
    ...
    const login = useCallback(async (email: string, password: string) => {
        ...
        try {
            ...
            if (profile?.role === 'client' && profile?.onboardingCompleted === false) {
            ...
            if (targetOfflineProfileId) {
                ...
                try {
                    ...
                    const migrateCollection = async (
                        ...
                        const sourceQuery = query(collection(db, collectionName), where(fieldName, '==', targetOfflineProfileId));
                        ...
                        sourceSnap.docs.forEach((sourceDoc) => {
                            batch.update(doc(db, collectionName, sourceDoc.id), {
                                [fieldName]: user.uid,
                                ...(extraFields || {}),
                            });
                        });
                    ...
                    await Promise.all([
                        migrateCollection('exercises', 'clientId', { updatedAt: serverTimestamp(), status: 'assigned' }),
                        migrateCollection('checkins', 'uid'),
                        migrateCollection('client_notes', 'clientId', { updatedAt: serverTimestamp() }),
                        migrateCollection('client_resources', 'clientId'),
                        migrateCollection('notifications', 'userId'),
                    ]);
