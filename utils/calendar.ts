...
import { AppointmentDetails, AppointmentSyncResult, CalendarLink, CalendarProvider, buildCalendarLinks, syncAppointmentToDevice } from '../modules/scheduling';
...
export async function syncAppointment(details: AppointmentDetails, options?: SyncAppointmentOptions): Promise<AppointmentSyncResult> {
    return syncAppointmentToDevice(details, {
        preferredProvider: options?.provider,
        silent: options?.silent,
    });
}

export function buildAppointmentLinks(details: AppointmentDetails): CalendarLink[] {
    return buildCalendarLinks(details);
}
